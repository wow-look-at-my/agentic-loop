...
func respTestProvider(t *testing.T, baseURL string, store bool) Provider {
	...
	return mustResponses(t, ResponsesConfig{
		ProviderConfig: ProviderConfig{BaseURL: baseURL, APIKey: "k", Retry: &RetryPolicy{MaxAttempts: 1}},
		Store:          store,
	})
}
...
func TestResponsesStreamsTextReasoningAndCalls(t *testing.T) {
	...
	require.True(t, comp.UsageReported)
	assert.Equal(t, 100, comp.Usage.PromptTokens, "input_tokens already includes the cached ones")
	assert.Equal(t, 20, comp.Usage.CompletionTokens)
	...
	assert.Equal(t, 0, *comp.Usage.CacheWriteTokens, "this API bills no separate cache-write class")
	require.NotNil(t, comp.ReasoningTokens)
	assert.Equal(t, 12, *comp.ReasoningTokens)
	assert.NotEmpty(t, comp.RawUsage, "the verbatim usage object survives for logging")
}
...
func TestResponsesUsageWithoutCacheDetailStaysUnknown(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Nil(t, comp.Usage.CacheReadTokens)
	assert.Nil(t, comp.Usage.CacheWriteTokens)
	assert.Nil(t, comp.ReasoningTokens)
...
func TestResponsesNonStreamingBody(t *testing.T) {
	...
	assert.Equal(t, 7, comp.Usage.PromptTokens)
	assert.NotEmpty(t, comp.RawUsage)
}
