...
// context. agentic.Tool calls are summarized inline; the result is rune-capped. It is
...
func generateContextSummary(ctx context.Context, p agentic.Provider, model string, msgs []agentic.Message, maxTokens int, extra map[string]any) (string, error) {
	transcript := RenderTranscript(msgs)
	if strings.TrimSpace(transcript) == "" {
		return "", nil
	}
	text, _, err := agentic.OneShot(ctx, p, agentic.Request{
		Model:  model,
