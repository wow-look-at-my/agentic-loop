...
- **There is NO turn cap, and adding one back is a regression.** No
  ...
  uselessly from one deep in a hard task, so it always fires at the worst
  possible moment: after the run has spent every call gathering context and
  right before the model writes any of it down — the most expensive failure
  mode available, because the whole investigation is paid for and then
  thrown away. What bounds a run is `ErrStuck` (repetition is the only
  mechanically detectable form of not-progressing) and the caller's `ctx`
  (wall-clock and spend, without discarding work in flight).
...
When something in this library (or anything it touches) is broken, the only
acceptable response is to fix the broken thing. Not a wrapper that avoids
it, not a sentinel that satisfies it, not a comment explaining how to live
with it. Fixed bugs are the deliverable. Workarounds are a bill sent to
everyone who comes later.

The turn cap is the case study. The loop capped every run at 10 model calls
and offered no way to say "no cap". The consumer needed uncapped runs, so
...
understood the defect completely — they had to, to route around it. The fix
was deleting a field. They wrote a paragraph instead.

What the workaround actually cost:

- **The bug stayed.** Every OTHER consumer still gets a silent hard 10.
- **It hid the bug.** The symptom was gone in one place, so nothing pushed
  anyone toward the real fix. Bugs get fixed because they hurt; a
  workaround is anaesthetic.
- **It cost days.** Agents were cut off mid-investigation, produced
  truncated reports, and got relaunched to redo work they had already done.
  Every one of those hours was spent re-deriving something the workaround's
  own author already knew.
- **It made the codebase lie.** `MaxTurns` looked like a supported knob.
  `1 << 30` looked like a considered value. Neither was true.

**Why the time cost is the part that matters.** A workaround is not a
neutral trade of elegance for speed — it converts a bounded, one-time cost
(fix it now, while you have the diagnosis in your head and the file open)
into an unbounded, recurring one paid by other people who do NOT have the
context, at unpredictable moments, usually under deadline. A bug you fix is
gone forever. A bug you route around is rediscovered from scratch by every
person who trips on it, and each rediscovery costs more than the original
fix, because they have to reverse-engineer the workaround before they can
even see the bug. Days spent that way buy nothing: no feature ships, no
question is answered, no code improves. It is the purest waste available in
software, and it is entirely avoidable by spending twenty minutes on the
real fix.
...
- **Never encode a bug's shape into a constant, sentinel, retry, sleep, or
  wrapper.** `1 << 30`, a magic default, an extra layer that exists only to
  undo a lower layer — all the same move.
- **A comment explaining a workaround is the alarm, not the solution.** If
  ...
  it.
- **The bug being in another layer, repo, or someone else's code is not an
  exemption.** Fix it there and take the extra step of getting it merged.
  If you genuinely cannot (no access), say so plainly, name the exact fix,
  and make the workaround loud and temporary — never quiet and permanent.
