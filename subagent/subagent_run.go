...
func (e *subagentTool) launched(ctx context.Context, callID string, in subagentArgs) {
	...
	res, spent := e.runGated(agentic.WithToolCallID(ctx, callID), in)
...
func (e *subagentTool) runGated(ctx context.Context, in subagentArgs) (agentic.ToolResult, []agentic.Usage) {
	if e.cfg.Provider == nil || e.cfg.Model == "" {
		return agentic.ToolResult{Content: "run_subagent is unavailable: no model is configured for the sub-agent", IsError: true}, nil
	}
	...
	if len(in.AllowedTools) > 0 {
		...
		if terr != "" {
			return agentic.ToolResult{Content: terr, IsError: true}, nil
		}
	...
	if errMsg != "" {
		return agentic.ToolResult{Content: errMsg, IsError: true}, spent
	}
...
func (e *subagentTool) runConfig(callID string, subTools agentic.Tools, granted bool) agentic.Config {
	...
	type cbs struct {
		toolCall   func(agentic.ToolCallEvent) error
		toolResult func(agentic.ToolResultEvent) error
		turnEnd    func(agentic.TurnEndEvent) error
	}
	c := &cbs{
		toolCall: func(ev agentic.ToolCallEvent) error {
			call := ev.Call
		...
		toolResult: func(ev agentic.ToolResultEvent) error {
	...
	c.turnEnd = func(ev agentic.TurnEndEvent) error {
...
// Ask always allows.
func (approveAll) Ask(context.Context, agentic.ToolCall) (agentic.Approval, error) {
	return agentic.Approval{OK: true}, nil
}
