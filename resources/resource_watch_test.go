...
func watcherOver(src ResourceSource, snaps ResourceSnapshots) agentic.ResourceWatcher {
...
func TestFirstPassIsABaseline(t *testing.T) {
	...
	assert.Equal(t, agentic.ResourceAdded, poll.Changes[0].Kind)
	assert.Equal(t, "FILE://A", poll.Changes[0].Label, "the title is the label when there is one")

	text := agentic.FormatResourceNotice(poll, "mcp_resource_diff")
...
func TestAChangeIdKeepsAnsweringItsOwnChange(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, poll.Changes, 1)
	assert.Equal(t, agentic.ResourceModified, poll.Changes[0].Kind)
...
func TestARemovedResourceIsReported(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, poll.Changes, 1)
	assert.Equal(t, agentic.ResourceRemoved, poll.Changes[0].Kind)
...
func TestTheNoticeCarriesIdsNotContent(t *testing.T) {
	...
	text := agentic.FormatResourceNotice(poll, "srv__mcp_resource_diff")
