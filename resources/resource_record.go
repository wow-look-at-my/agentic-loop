...
func (w *resourceWatcher) record(ctx context.Context, kind string, c capture, before ResourceSnapshot, existed bool) (agentic.ResourceChange, error) {
	...
	if err != nil {
		return agentic.ResourceChange{}, fmt.Errorf("recording a resource change: %w", err)
	}
	if err := w.snapshots.PutSnapshot(ctx, ResourceSnapshot{
		...
		return agentic.ResourceChange{}, fmt.Errorf("updating a watched resource: %w", err)
	}
	return agentic.ResourceChange{
		ChangeID: id,
		Server:   c.sourceName,
		URI:      c.uri,
		Label:    c.label,
...
func (w *resourceWatcher) recordRemoval(ctx context.Context, before ResourceSnapshot) (agentic.ResourceChange, error) {
	...
	if err != nil {
		return agentic.ResourceChange{}, fmt.Errorf("recording a resource removal: %w", err)
	}
	if err := w.snapshots.DeleteSnapshot(ctx, before.SourceID, before.URI); err != nil {
		return agentic.ResourceChange{}, fmt.Errorf("dropping a watched resource: %w", err)
	}
	return agentic.ResourceChange{
		ChangeID: id,
		Server:   sourceName,
		...
		Label:    before.Label,
		Kind:     agentic.ResourceRemoved,
		Summary:  "no longer advertised (was " + agentic.HumanSize(before.ByteSize) + ")",
	}, nil
}
...
func summarizeResourceChange(kind, before, after string, beforeBytes, afterBytes int64, binary bool) string {
	if binary {
		if kind == agentic.ResourceAdded {
			return "binary, " + agentic.HumanSize(afterBytes)
		}
		return "binary, " + agentic.HumanSize(beforeBytes) + " -> " + agentic.HumanSize(afterBytes) + " (content not captured)"
	}
	if kind == agentic.ResourceAdded {
		return agentic.HumanSize(afterBytes) + ", " + agentic.Plural(agentic.CountLines(after), "line", "lines")
	}
	added, removed := agentic.CountLineChanges(before, after)
	return agentic.HumanSize(beforeBytes) + " -> " + agentic.HumanSize(afterBytes) +
...
func captureNote(binary, truncated bool, maxBytes int) string {
	...
	case truncated:
		return "the captured content was cut at " + agentic.HumanSize(int64(maxBytes)) +
			", so the diff covers only the first " + agentic.HumanSize(int64(maxBytes)) + " of this resource"
	default:
		return ""
	}
}
