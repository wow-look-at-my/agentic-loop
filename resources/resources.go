...
func FormatResourceNotice(poll ResourcePoll, diffTool string) string {
	...
	if n := len(poll.Changes); n > 0 {
		...
		switch {
		case poll.Baseline:
			b.WriteString(agentic.Plural(n, "MCP resource is", "MCP resources are") +
				" available on the connected servers and now being watched:")
		default:
			b.WriteString(agentic.Plural(n, "MCP resource changed", "MCP resources changed") + " since the last turn:")
		}
