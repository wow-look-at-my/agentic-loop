...
// wfCall builds a web_fetch agentic.ToolCall with the given JSON arguments.
...
<h1>Big   Title</h1><p>Hello &amp; welcome.<br>Line two.</p></body></html>`))
	...
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL + "/page"})))
...
func TestWebFetchTruncationNote(t *testing.T) {
	...
	exec := NewWebFetchTool(WebFetchConfig{})
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL})))
	require.NoError(t, err)
	assert.False(t, res.IsError)
...
func TestWebFetchConnectionError(t *testing.T) {
	...
	srv.Close()
	exec := NewWebFetchTool(WebFetchConfig{})
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": url})))
	require.NoError(t, err)
	assert.True(t, res.IsError)
...
func TestWebFetchURLValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": tc.url})))
			require.NoError(t, err)
			assert.True(t, res.IsError)
...
func TestWebFetchBlockHook(t *testing.T) {
	...
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL + "/repo"})))
	require.NoError(t, err)
	assert.True(t, res.IsError)
	...
	defer srv2.Close()
	res, err = allow.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv2.URL})))
	require.NoError(t, err)
	assert.False(t, res.IsError)
...
func TestWebFetchSummaryPath(t *testing.T) {
	...
	provider := &testkit.ScriptProvider{Steps: []testkit.ScriptStep{{Comp: testkit.AssistantComp("  a fine summary  ")}}}
	exec := NewWebFetchTool(WebFetchConfig{
		Provider: provider, Model: "m", MaxTokens: 256, Extra: map[string]any{"temperature": 0.1},
	})
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL + "/doc", "summary_prompt": "list the key points"})))
	require.NoError(t, err)
	assert.False(t, res.IsError)
...
func TestWebFetchOnCompletionReportsTheSummaryCall(t *testing.T) {
	...
	cost := 0.25
	comp := testkit.AssistantComp("a fine summary")
	comp.UsageReported = true
	comp.CostUsd = &cost
	provider := &testkit.ScriptProvider{Steps: []testkit.ScriptStep{{Comp: comp}}}
	var got []*agentic.Completion
	exec := NewWebFetchTool(WebFetchConfig{
		Provider: provider, Model: "m",
		OnCompletion: func(c *agentic.Completion) { got = append(got, c) },
	})

	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL, "summary_prompt": "sum"})))
	require.NoError(t, err)
	assert.False(t, res.IsError)
	require.Len(t, got, 1)
...
func TestWebFetchOnCompletionFiresOnlyForARealCall(t *testing.T) {
	...
	var got []*agentic.Completion
	record := func(c *agentic.Completion) { got = append(got, c) }

	exec := NewWebFetchTool(WebFetchConfig{
		Provider: &testkit.ScriptProvider{}, Model: "m", OnCompletion: record,
	})
	_, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL})))
	require.NoError(t, err)
	assert.Empty(t, got, "no summary was asked for, so no call was made")

	partial := &agentic.Completion{Message: agentic.Message{Role: agentic.RoleAssistant, Content: "half a sum"}, Usage: agentic.Usage{TotalTokens: 7}}
	exec = NewWebFetchTool(WebFetchConfig{
		Provider:     &testkit.ScriptProvider{Steps: []testkit.ScriptStep{{Comp: partial, Err: context.Canceled}}},
		Model:        "m",
		OnCompletion: record,
	})
	res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL, "summary_prompt": "sum"})))
...
func TestWebFetchSummaryErrors(t *testing.T) {
	...
	t.Run("summary call fails", func(t *testing.T) {
		provider := &testkit.ScriptProvider{Steps: []testkit.ScriptStep{{Err: &agentic.APIError{Status: 500, Body: "down"}}}}
	...
	t.Run("summary empty", func(t *testing.T) {
		provider := &testkit.ScriptProvider{Steps: []testkit.ScriptStep{{Comp: testkit.AssistantComp("   ")}}}
		exec := NewWebFetchTool(WebFetchConfig{Provider: provider, Model: "m"})
		res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": srv.URL, "summary_prompt": "sum"})))
		require.NoError(t, err)
		assert.True(t, res.IsError)
...
func TestWebFetchTika(t *testing.T) {
	...
	t.Run("extraction wins", func(t *testing.T) {
		...
		res, err := exec.Execute(context.Background(), wfCall(jsontest.Must(jsontest.Obj{"url": page.URL})))
		require.NoError(t, err)
		assert.False(t, res.IsError)
...
func TestWebFetchTextHelpers(t *testing.T) {
	...
	t.Run("TruncateRunes", func(t *testing.T) {
		s, cut := agentic.TruncateRunes("hello", 10)
		assert.Equal(t, "hello", s)
		assert.False(t, cut)
		s, cut = agentic.TruncateRunes("hello world", 5)
		assert.Equal(t, "hello", s)
		assert.True(t, cut)
		s, cut = agentic.TruncateRunes("hello", 0)
		assert.Equal(t, "hello", s)
