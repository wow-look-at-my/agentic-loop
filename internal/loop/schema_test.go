...
func TestInferSchemaPanicsOnAMisdeclaredArgument(t *testing.T) {
	...
	assert.PanicsWithValue(t, "agentic: listOfStructs.Items is []loop.noTag; a tool argument list must hold scalars",
		func() { InferSchema[listOfStructs]() })
	assert.PanicsWithValue(t, `agentic: cannot constrain unknown property "nope" on loop.schemaFixture`,
