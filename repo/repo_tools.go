...
var repoReadSchema = agentic.EnumSchema[repoReadArgs](map[string][]string{
...
func NewRepoTools(cfg RepoToolsConfig) agentic.Tools {
	if cfg.GitHub == nil {
		return nil
	}
	...
	return agentic.Tools{
		...
		approvalTool{agentic.NewTool(agentic.ToolDecl{
...
func wrapRepoTool(run func(context.Context, json.RawMessage) agentic.ToolResult) func(context.Context, json.RawMessage) (agentic.ToolResult, error) {
	return func(ctx context.Context, args json.RawMessage) (agentic.ToolResult, error) {
...
var repoReadWhats = map[string]func(*repoTools, context.Context, repoReadArgs) agentic.ToolResult{
...
func (e *repoTools) repoRead(ctx context.Context, args json.RawMessage) agentic.ToolResult {
	var in repoReadArgs
	if err := json.Unmarshal(args, &in); err != nil {
		return agentic.ToolResult{Content: "invalid repo_read arguments: " + err.Error(), IsError: true}
	}
	what := strings.ToLower(strings.TrimSpace(in.What))
	if what == "" {
		return agentic.ToolResult{Content: `repo_read requires "what": one of ` + repoReadWhatList, IsError: true}
	}
	...
	if !ok {
		...
		return agentic.ToolResult{Content: fmt.Sprintf("repo_read: unknown what %q; must be one of %s", in.What, repoReadWhatList), IsError: true}
	}
