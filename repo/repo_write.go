...
var repoFileWriteSchema = agentic.InferSchema[repoFileWriteArgs]()
...
func (e *repoTools) runWrite(ctx context.Context, toolName, cacheKey string, attempt func(token string) (string, error)) agentic.ToolResult {
	...
	if len(order) == 0 {
		if len(e.gh.tokens) > 0 {
			...
			return agentic.ToolResult{Content: noModelWriteTokensMsg, IsError: true}
		}
		return agentic.ToolResult{Content: toolName + ` mutates GitHub and is never attempted unauthenticated — add a GitHub personal access token with write access in Settings -> github and enable "model can write" on it first.`, IsError: true}
	}
	...
	for _, att := range order {
		...
		if err == nil {
			...
			return agentic.ToolResult{Content: text}
		}
		var auth GitHubAuthError
		if errors.As(err, &auth) {
		...
		if errors.As(err, &fatal) {
			return agentic.ToolResult{Content: fatal.msg, IsError: true}
		}
		return agentic.ToolResult{Content: toolName + " request failed: " + err.Error(), IsError: true}
	}
	return agentic.ToolResult{Content: e.explainExhaustedWrite(ctx, toolName, cacheKey, order, bestAuth), IsError: true}
}
...
func (e *repoTools) fileWrite(ctx context.Context, args json.RawMessage) agentic.ToolResult {
	var in repoFileWriteArgs
	if err := json.Unmarshal(args, &in); err != nil {
		return agentic.ToolResult{Content: "invalid repo_file_write arguments: " + err.Error(), IsError: true}
	}
...
func (e *repoTools) prCreate(ctx context.Context, args json.RawMessage) agentic.ToolResult {
	var in repoPRCreateArgs
	if err := json.Unmarshal(args, &in); err != nil {
		return agentic.ToolResult{Content: "invalid repo_pr_create arguments: " + err.Error(), IsError: true}
	}
	...
	if in.Title == "" || in.Head == "" {
		return agentic.ToolResult{Content: `repo_pr_create requires non-empty "title" and "head" (the branch carrying the changes), e.g. {"org":"octocat","repo":"hello-world","title":"Fix typo","head":"fix-typo"}`, IsError: true}
	}
	draft := true
	if in.Draft != nil {
		draft = *in.Draft
