...
func (e *repoTools) commitList(ctx context.Context, in repoReadArgs) agentic.ToolResult {
	...
	if err != nil {
		return agentic.ToolResult{Content: "repo_read what=commits request failed: " + err.Error(), IsError: true}
	}
	if res.status < 200 || res.status >= 300 {
		return agentic.ToolResult{Content: DescribeResourceFailure("list commits of", RepoPath(in.Org, in.Repo, ""), res, len(e.gh.tokens)), IsError: true}
	}
	var commits []ghCommitEntry
	if uerr := json.Unmarshal(res.body, &commits); uerr != nil {
		return agentic.ToolResult{Content: "repo_read what=commits: could not parse GitHub's response: " + uerr.Error(), IsError: true}
	}
...
func (e *repoTools) commitRead(ctx context.Context, in repoReadArgs) agentic.ToolResult {
	...
	if sha == "" {
		return agentic.ToolResult{Content: `repo_read what=commit requires "sha" (a commit SHA, e.g. from what=commits)`, IsError: true}
	}
	...
	if err != nil {
		return agentic.ToolResult{Content: "repo_read what=commit request failed: " + err.Error(), IsError: true}
	}
	if res.status < 200 || res.status >= 300 {
		return agentic.ToolResult{Content: DescribeResourceFailure("read", "commit "+sha+" of "+RepoPath(in.Org, in.Repo, ""), res, len(e.gh.tokens)), IsError: true}
	}
	diff, truncated := agentic.TruncateRunes(string(res.body), RepoDiffMaxRunes)
