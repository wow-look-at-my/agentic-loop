...
func (e *repoTools) issueList(ctx context.Context, in repoReadArgs) agentic.ToolResult {
	...
	if err != nil {
		return agentic.ToolResult{Content: "repo_read what=issues: " + err.Error(), IsError: true}
	}
	q := url.Values{}
	q.Set("state", state)
	...
	if ferr != nil {
		return agentic.ToolResult{Content: "repo_read what=issues request failed: " + ferr.Error(), IsError: true}
	}
	if res.status < 200 || res.status >= 300 {
		return agentic.ToolResult{Content: DescribeResourceFailure("list issues of", RepoPath(in.Org, in.Repo, ""), res, len(e.gh.tokens)), IsError: true}
	}
	var issues []ghIssue
	if uerr := json.Unmarshal(res.body, &issues); uerr != nil {
		return agentic.ToolResult{Content: "repo_read what=issues: could not parse GitHub's response: " + uerr.Error(), IsError: true}
	}
...
func (e *repoTools) issueRead(ctx context.Context, in repoReadArgs) agentic.ToolResult {
	if err := requireOrgRepo("repo_read what=issue", in.Org, in.Repo); err != nil {
		return agentic.ToolResult{Content: err.Error(), IsError: true}
	}
	if in.Number <= 0 {
		return agentic.ToolResult{Content: `repo_read what=issue requires a positive "number" (an issue number, e.g. from what=issues)`, IsError: true}
	}
	...
	if err != nil {
		return agentic.ToolResult{Content: "repo_read what=issue request failed: " + err.Error(), IsError: true}
	}
	if res.status < 200 || res.status >= 300 {
	...
	if uerr := json.Unmarshal(res.body, &is); uerr != nil {
		return agentic.ToolResult{Content: "repo_read what=issue: could not parse GitHub's response: " + uerr.Error(), IsError: true}
	}
	if is.isPR() {
		return agentic.ToolResult{Content: fmt.Sprintf("#%d in %s/%s is a pull request, not an issue — use repo_read what=pr to read it.", in.Number, in.Org, in.Repo), IsError: true}
	}
