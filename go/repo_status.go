...
func (e *repoTools) statusRead(ctx context.Context, in repoReadArgs) ToolResult {
	...
	if ref == "" {
		...
		if err != nil {
			return ToolResult{Content: "repo_read what=status: could not resolve the default branch: " + err.Error(), IsError: true}
		}
		ref = branch
	}
	...
	return ToolResult{Content: text + tokenExpiryDetail(res, time.Now())}
}
...
func formatFailureDetail(c ghCheckRun) string {
	...
	if s := strings.TrimSpace(c.Output.Summary); s != "" {
		summary, truncated := TruncateRunes(s, statusSummaryMaxRunes)
...
func (e *repoTools) checkRunRead(ctx context.Context, in repoReadArgs) ToolResult {
	...
	return ToolResult{Content: formatCheckRun(in.Org, in.Repo, c, annotations, annNote)}
}
...
func formatCheckRun(org, repo string, c ghCheckRun, annotations []ghAnnotation, annNote string) string {
	...
	if txt := strings.TrimSpace(c.Output.Text); txt != "" {
		body, truncated := TruncateRunes(txt, checkRunTextMaxRunes)
		fmt.Fprintf(&b, "\n%s\n", body)
		if truncated {
