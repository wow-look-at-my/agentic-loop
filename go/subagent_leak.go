...
func subagentReport(final string) ToolResult {
	...
	if len([]rune(clean)) < minReportRunes {
		return ToolResult{Content: SubagentNoReportText, IsError: true}
	}
	return ToolResult{Content: clean + SubagentCutOffNote, IsError: true}
}
