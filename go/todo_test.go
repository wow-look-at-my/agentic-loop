...
// returns the store and a map from advertised name to the actual Tool.
func todoTools(t *testing.T, rec *recordingTodos) map[string]Tool {
	t.Helper()
	...
	require.Len(t, byName, 4, "the four tools are separate Tools in one flat slice")
	return byName
}

// run executes one tool against the real Execute, asserting it is not a Go error.
func run(t *testing.T, tool Tool, args string) ToolResult {
	t.Helper()
