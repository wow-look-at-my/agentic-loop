...
func TestOpenAIStreamDecode(t *testing.T) {
	...
	assert.Equal(t, 30, comp.Usages[5].TotalTokens, "the reasoning-surplus report survives as sent")
	require.NotNil(t, comp.Usages[5].CacheReadTokens)
	assert.Equal(t, 8, *comp.Usages[5].CacheReadTokens)
	assert.Equal(t, 0, comp.Usages[6].TotalTokens, "a regressing final report is reported, not swallowed")
...
func TestOpenAIPartialOnMidStreamDeath(t *testing.T) {
	...
	assert.Equal(t, 5, firstUsage(comp).TotalTokens, "last usage snapshot kept")
}
