...
func (e *repoTools) prList(ctx context.Context, in repoReadArgs) ToolResult {
	if err := requireOrgRepo("repo_read what=prs", in.Org, in.Repo); err != nil {
		return ToolResult{Content: err.Error(), IsError: true}
	}
	state, err := parseListState(in.State)
	if err != nil {
		return ToolResult{Content: "repo_read what=prs: " + err.Error(), IsError: true}
	}
	...
	if uerr := json.Unmarshal(res.body, &pulls); uerr != nil {
		return ToolResult{Content: "repo_read what=prs: could not parse GitHub's response: " + uerr.Error(), IsError: true}
	}
	return ToolResult{Content: formatPRList(in.Org, in.Repo, state, pulls)}
}
...
func (e *repoTools) prRead(ctx context.Context, in repoReadArgs) ToolResult {
	if err := requireOrgRepo("repo_read what=pr", in.Org, in.Repo); err != nil {
		return ToolResult{Content: err.Error(), IsError: true}
	}
	if in.Number <= 0 {
		return ToolResult{Content: `repo_read what=pr requires a positive "number" (a pull request number, e.g. from what=prs)`, IsError: true}
	}
	...
	if uerr := json.Unmarshal(res.body, &pr); uerr != nil {
		return ToolResult{Content: "repo_read what=pr: could not parse GitHub's response: " + uerr.Error(), IsError: true}
	}
	...
	return ToolResult{Content: formatPR(in.Org, in.Repo, pr, files, filesErr, diff, diffNote)}
}
