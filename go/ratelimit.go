// ratelimit.go implements the library's fixed-rate request limiter, a
// provider-side throttle for staying under a provider's per-minute request
// cap (e.g. Novita's 30 requests/minute). A RateLimiter spaces request STARTS
// evenly -- at most n per minute -- so a hard provider cap is never exceeded.
//
