...
// uselessly" from "deep in a hard task", so it always fires at the worst
// moment: after the model has spent every call gathering context and just
// before it writes any of it down. What bounds a run instead is ErrStuck (a
// model repeating itself is the only mechanically detectable kind of "not
// progressing") and the caller's ctx, which bounds wall-clock and spend
// without discarding work in flight.
...
// DeniedMessage), and the event callbacks. There is deliberately no turn
// cap -- see the note above wrapUpInstruction for what bounds a run instead.
//
